    ...
    <div class="category_box">
        ...
        <div class="overflow-auto">
            <ul class="category_list  overflow-auto" id="add_product_category_search_list3">
                ...
                where('child_two',$all_sub_cat->child_two)->whereNotNull('child_three')
        ->select('child_three')->distinct()->latest()->get();

                ?>
                @if(count($check_next_data_available_or_not) == 0)
                <li data-childtwo="{{ $all_sub_cat->child_two }}"  class="dynamic_childtwo">{{ $all_sub_cat->child_two }}<span> </span>
                </li>

@else
<li data-childtwo="{{ $all_sub_cat->child_two }}" id="childtwo_name{{ $key+1 }}" class="dynamic_childtwo">{{ $all_sub_cat->child_two }}<span> <i class="bx bx-right-arrow-alt"></i> </span>
</li>
@endif
