    ...
    <div class="category_box">
        ...
        <div class="overflow-auto">
            <ul class="category_list  overflow-auto" id="add_product_category_search_list7">
                ...
                where('child_six',$all_sub_cat->child_six)->whereNotNull('child_seven')
        ->select('child_seven')->distinct()->latest()->get();

                ?>
                @if(count($check_next_data_available_or_not) == 0)
                <li data-childsix="{{ $all_sub_cat->child_six }}"  class="dynamic_childsix">{{ $all_sub_cat->child_six }}<span>  </span>
                </li>

@else
<li data-childsix="{{ $all_sub_cat->child_six }}" id="childsix_name{{ $key+1 }}" class="dynamic_childsix">{{ $all_sub_cat->child_six }}<span> <i class="bx bx-right-arrow-alt"></i> </span>
</li>
@endif
