    ...
    <div class="category_box">
        ...
        <div class="overflow-auto">
            <ul class="category_list  overflow-auto" id="add_product_category_search_list2">
                ...
                where('child_one',$all_sub_cat->child_one)->whereNotNull('child_two')
        ->select('child_two')->distinct()->latest()->get();

                ?>
                @if(count($check_next_data_available_or_not) == 0)
                <li data-childone="{{ $all_sub_cat->child_one }}"  class="dynamic_childone">{{ $all_sub_cat->child_one }}<span>  </span>
                </li>

@else
<li data-childone="{{ $all_sub_cat->child_one }}" id="childone_name{{ $key+1 }}" class="dynamic_childone">{{ $all_sub_cat->child_one }}<span> <i class="bx bx-right-arrow-alt"></i> </span>
</li>
@endif
