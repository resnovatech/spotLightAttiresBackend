    ...
    <div class="category_box">
        ...
        <div class="overflow-auto">
            <ul class="category_list  overflow-auto" id="add_product_category_search_list8">
                ...
                <?php
                $check_next_data_available_or_not = DB::table('categories')->
                where('child_seven',$all_sub_cat->child_seven)->whereNotNull('child_eight')
        ->select('child_eight')->distinct()->latest()->get();

                ?>
                @if(count($check_next_data_available_or_not) == 0)
                <li data-childseven="{{ $all_sub_cat->child_seven }}"  class="dynamic_childseven">{{ $all_sub_cat->child_seven}}<span>  </span>
                </li>

@else
<li data-childseven="{{ $all_sub_cat->child_seven }}" id="childseven_name{{ $key+1 }}" class="dynamic_childseven">{{ $all_sub_cat->child_seven}}<span> <i class="bx bx-right-arrow-alt"></i> </span>
</li>
@endif
