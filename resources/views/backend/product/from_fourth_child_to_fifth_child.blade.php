    ...
    <div class="category_box">
        ...
        <div class="overflow-auto">
            <ul class="category_list  overflow-auto" id="add_product_category_search_list6">
                ...
                where('child_five',$all_sub_cat->child_five)->whereNotNull('child_six')
        ->select('child_six')->distinct()->latest()->get();

                ?>
                @if(count($check_next_data_available_or_not) == 0)
                <li data-childfive="{{ $all_sub_cat->child_five }}"  class="dynamic_childfive">{{ $all_sub_cat->child_five }}<span>  </span>
                </li>

@else
<li data-childfive="{{ $all_sub_cat->child_five }}" id="childfive_name{{ $key+1 }}" class="dynamic_childfive">{{ $all_sub_cat->child_five }}<span> <i class="bx bx-right-arrow-alt"></i> </span>
</li>
@endif
