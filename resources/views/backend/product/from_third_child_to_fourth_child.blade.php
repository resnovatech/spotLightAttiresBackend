    ...
    <div class="category_box">
        ...
        <div class="overflow-auto">
            <ul class="category_list  overflow-auto" id="add_product_category_search_list5">
                ...
                where('child_four',$all_sub_cat->child_four)->whereNotNull('child_five')
        ->select('child_five')->distinct()->latest()->get();

                ?>
                @if(count($check_next_data_available_or_not) == 0)
                <li data-childfour="{{ $all_sub_cat->child_four }}"  class="dynamic_childfour">{{ $all_sub_cat->child_four }}<span>  </span>
                </li>

@else
<li data-childfour="{{ $all_sub_cat->child_four }}" id="childfour_name{{ $key+1 }}" class="dynamic_childfour">{{ $all_sub_cat->child_four }}<span> <i class="bx bx-right-arrow-alt"></i> </span>
</li>
@endif
