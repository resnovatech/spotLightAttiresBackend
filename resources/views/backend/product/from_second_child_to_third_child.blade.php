    ...
    <div class="category_box">
        ...
        <div class="overflow-auto">
            <ul class="category_list  overflow-auto" id="add_product_category_search_list4">
                ...
                where('child_three',$all_sub_cat->child_three)->whereNotNull('child_four')
        ->select('child_four')->distinct()->latest()->get();

                ?>
                @if(count($check_next_data_available_or_not) == 0)
                <li data-childthree="{{ $all_sub_cat->child_three }}"  class="dynamic_childthree">{{ $all_sub_cat->child_three }}<span>  </span>
                </li>

@else
<li data-childthree="{{ $all_sub_cat->child_three }}" id="childthree_name{{ $key+1 }}" class="dynamic_childthree">{{ $all_sub_cat->child_three }}<span> <i class="bx bx-right-arrow-alt"></i> </span>
</li>
@endif
