...
->where('delivery_status','Cancelled')->latest()->count();
        ...
        $total_serial_number123= $total_datac/10;

        if (is_float($total_serial_number123)){
            $total_serial_numberc = ceil($total_serial_number123);

        }else{
            $total_serial_numberc = $total_serial_number123;
        }

        $invoice_listc = DB::table('invoices')
        ->where('delivery_status','Cancelled')->latest()->limit(10)->get();

?>
<div class="row" id="main_content_tablec">
    <div class="table-responsive">
        <table class="table align-middle table-nowrap table-check">
            ...
            <tbody>
                @foreach($invoice_listc as $all_invoice_list)
            <tr>
                <td>
                    <div class="form-check font-size-16">
...
<!-- end modal -->
                ...
                <td>
                    ...
                    <button type="button" class="btn btn-primary btn-sm btn-rounded" data-bs-toggle="modal" data-bs-target=".orderdetailsModalc{{ $all_invoice_list->id }}">
                        View Details
                    </button>
                    <!-- Modal -->
<div class="modal fade orderdetailsModalc{{ $all_invoice_list->id }}" tabindex="-1" role="dialog" aria-labelledby=orderdetailsModalLabel" aria-hidden="true">
<div class="modal-dialog modal-dialog-centered" role="document">
<div class="modal-content">
<div class="modal-header">
...
@endif
    ...
    <p>Showing 1 to {{count($invoice_listc)}} of {{$total_datac}} entries</p>
   <!--pagination start -->
@include('backend.invoice.normal_pagination_cancelled')
   <!--pagination end -->
    </div>
