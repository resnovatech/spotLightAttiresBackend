...
->where('delivery_status','Delivered')->latest()->count();
        ...
        $total_serial_number12= $total_datad/10;

        if (is_float($total_serial_number12)){
            $total_serial_numberd = ceil($total_serial_number12);

        }else{
            $total_serial_numberd = $total_serial_number12;
        }

        $invoice_listd = DB::table('invoices')
        ->where('delivery_status','Delivered')->latest()->limit(10)->get();

?>
<div class="row" id="main_content_tabled">
    <div class="table-responsive">
        <table class="table align-middle table-nowrap table-check">
            ...
            <tbody>
                @foreach($invoice_listd as $all_invoice_list)
            <tr>
                <td>
                    <div class="form-check font-size-16">
...
<!-- end modal -->
                ...
                <td>
                    ...
                    <button type="button" class="btn btn-primary btn-sm btn-rounded" data-bs-toggle="modal" data-bs-target=".orderdetailsModaldd{{ $all_invoice_list->id }}">
                        View Details
                    </button>
                    <!-- Modal -->
<div class="modal fade orderdetailsModaldd{{ $all_invoice_list->id }}" tabindex="-1" role="dialog" aria-labelledby="orderdetailsModalLabel" aria-hidden="true">
<div class="modal-dialog modal-dialog-centered" role="document">
<div class="modal-content">
<div class="modal-header">
...
@endif
    ...
    <p>Showing 1 to {{count($invoice_listd)}} of {{$total_datad}} entries</p>
   <!--pagination start -->
@include('backend.invoice.normal_pagination_delivered')
   <!--pagination end -->
    </div>
