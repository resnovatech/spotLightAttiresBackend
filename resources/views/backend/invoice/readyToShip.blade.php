...
->where('delivery_status','Ready To Ship')->latest()->count();
        ...
        $total_serial_number1= $total_datar/10;

        if (is_float($total_serial_number1)){
            $total_serial_numberr = ceil($total_serial_number1);

        }else{
            $total_serial_numberr = $total_serial_number1;
        }

        $invoice_listr = DB::table('invoices')
        ->where('delivery_status','Ready To Ship')->latest()->limit(10)->get();
...
<div class="row" id="main_content_tabler">
    <div class="table-responsive">
        <table class="table align-middle table-nowrap table-check">
            ...
            <tbody>
                @foreach($invoice_listr as $all_invoice_list)
            <tr>
                <td>
                    <div class="form-check font-size-16">
...
<!-- end modal -->
                ...
                <td>
                    ...
                    <button type="button" class="btn btn-primary btn-sm btn-rounded" data-bs-toggle="modal" data-bs-target=".orderdetailsModalrrr{{ $all_invoice_list->id }}">
                        View Details
                    </button>
                    <!-- Modal -->
<div class="modal fade orderdetailsModalrrr{{ $all_invoice_list->id }}" tabindex="-1" role="dialog" aria-labelledby=orderdetailsModalLabel" aria-hidden="true">
<div class="modal-dialog modal-dialog-centered" role="document">
<div class="modal-content">
<div class="modal-header">
...
@endif
    ...
    <p>Showing 1 to {{count($invoice_listr)}} of {{$total_datar}} entries</p>
   <!--pagination start -->
@include('backend.invoice.normal_pagination_readyToShip')
   <!--pagination end -->
    </div>
