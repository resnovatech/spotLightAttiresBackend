...
@section('title')
Blog Category information | {{ $ins_name }}
@endsection
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0">Blog Category  Information</h4>
    ...
    <div class="col-12">
        ...
        <p class="card-title-desc">You can <code>Show, Hide & Edit</code> all Blog Category </p>
    </div>
</div>
<!-- search bar --->
<div class="row">
...
<div class="row mt-2">
    ...
    <div class="col-sm-6">
        <div class="text-sm-end">
            <button type="button"
                ...
                <i class="mdi mdi-plus me-1"></i> Add New Blog Category
            </button>
        </div>
            <!-- Modal -->
    ...
    <h5 class="modal-title" id="exampleModalLabel">Add Blog Category  Information</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
    </button>
    </div>
    <div class="modal-body">
        <form class="mt-3" method="Post" action="{{ route('blog_category_store') }}" enctype="multipart/form-data">
            @csrf
            <div class="row">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    ...
                                    @if (Auth::guard('admin')->user()->can('blog_category_update'))
...
<button class="btn btn-transparent btn-sm text-success" data-bs-toggle="modal" data-bs-target=".bs-example-modal-lg{{ $all_attribute->id }}">
    ...
    <!-- Modal -->
    @include('backend.blog_cat.edit')
@endif
    @if (Auth::guard('admin')->user()->can('blog_category_delete'))
                                        ...
                                        <button class=" btn btn-transparent btn-sm text-danger"  onclick="deleteTag({{ $all_attribute->id}})"><i
                                                    ...
                                                    <form id="delete-form-{{ $all_attribute->id }}" action="{{ route('blog_category_delete',$all_attribute->id) }}" method="POST" style="display: none;">
...
@endforeach
                ...
                @include('backend.blog_cat.normal_pagination')
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript" src="{{ asset('/') }}public/custom_js/blog_cat_search.js"></script>
<script type="text/javascript" src="{{ asset('/') }}public/custom_js/blog_cat.js"></script>
