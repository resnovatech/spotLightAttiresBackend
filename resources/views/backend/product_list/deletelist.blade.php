...
$total_data_delete =DB::table('main_products')
                     ->where('action_button','deleted')
                     ->latest()->count();

$total_serial_number1_delete= $total_data_delete/10;
...
<div class="row mt-4 mb-2">
    <div class="col-sm-6">
        <div class="d-flex flex-wrap gap-2">
            <p class="horizontal-center">Selected: <span id="delete_numberOfChecked"> 0</span></p>

            <button  id="delete_delete_button" class="btn btn-outline-secondary waves-effect" disabled>
                Delete
            </button>
        </div>
    </div>
...
<div class="view" id="main_content_table_delete">
    <div class="wrapper" >
        <table class="table table-striped">
            ...
            <tr>
                <th class="sticky-coll first-col-manage align-middle">
                    <div class="form-check font-size-16">
                        <input class="form-check-input" type="checkbox" id="delete_master">
            ...
            <tbody>
                @foreach($delete_product_list as $key=>$all_product_list)
            <tr>
                <td class="sticky-coll first-col-manage">
                    <div class="form-check font-size-16">
                        <input class="form-check-input delete_sub_chk" value="{{$all_product_list->id}}"  data-id="{{$all_product_list->id}}" type="checkbox" id="orderidcheck01">
<label class="form-check-label" for="orderidcheck01"></label>
                ...
                <td class="sticky-coll-last forth-col-manage">
                    <div class="d-flex gap-3">
                        <a href="{{ route('product_list_restore_from_deletelist',$all_product_list->id) }}" class="text-success"><i class="mdi mdi-backup-restore font-size-18"></i></a>
                        @if (Auth::guard('admin')->user()->can('product_delete'))
                        <a href="javascript:void(0);"  onclick="pdeleteTag({{ $all_product_list->id}})" class="text-danger"><i class="mdi mdi-delete font-size-18"></i></a>

                        <form id="delete-form-{{ $all_product_list->id }}" action="{{ route('product_list_single_deletelist_delete',$all_product_list->id) }}" method="POST" style="display: none;">
     ...
     <!--pagination all page-->
@include('backend.product_list.deletelist_normal_pagination')
